...
namespace org::modcpp::hashing {
  ...
  Md5Hasher::Md5Hasher(cstring text) {
    init();
    put(text);
  }

  Md5Hasher::Md5Hasher(const String &text) {
    init();
  ...
  /**
   * @brief decodes input (unsigned char) into output (uint32_t).
   *        Assumes len is a multiple of 4.
   */
  void Md5Hasher::decode(uint32_t output[], const uint8_t input[], SizeType len) {
  ...
  /**
   * @brief encodes input (uint32_t) into output (unsigned char). Assumes len is
   *        a multiple of 4.
   */
  void Md5Hasher::encode(uint8_t output[], const uint32_t input[], SizeType len) {
    for (SizeType i = 0, j = 0; j < len; i++, j += 4) {
  ...
  void Md5Hasher::transform(const uint8_t block[blockSize]) {
  ...
  void Md5Hasher::put(const unsigned char * const input, SizeType length) {
    // compute number of bytes mod 64
    SizeType index = count[0] / 8 % blockSize;
  ...
  void Md5Hasher::finalize() {
    ...
    if (!finalized) {
      ...
      SizeType index = count[0] / 8 % 64;
      SizeType padLen = (index < 56) ? (56 - index) : (120 - index);
      put(padding, padLen);
  ...
  String Md5Hasher::hash() {
    ...
    return String::copyOf(buf);
  }

  String Md5Hasher::hash(const String& text) {
    put(text.begin(), text.length());
  ...
  /**
  std::ostream& operator<<(std::ostream& out, Md5Hasher md5Hasher)
  {
    return out << md5Hasher.hash();
  }**/
